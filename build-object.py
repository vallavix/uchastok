...
# Приход материалов по накладным. Ключ — начало названия позиции в спецификации.
# Значение: число (одной строкой) либо список поставок
# [{'q': сколько, 'd': 'ГГГГ-ММ-ДД', 'doc': '№ накладной'}, ...].
# Приложение показывает сумму и раскрывает историю по клику на названии.
RECEIVED = {
    'b1': {
        # «П/Г» в записке — это IP44, влагозащищённые для санузлов
        'Розетка с защ. шторками безоп. скрытой проводки, белая, 16А, IP44': 290,
        # «IP20» — обычные с защитными шторками
        'Розетка с защитными шторками безопасности': 4008,
        # 724 горизонтальных + 265 вертикальных: материал тот же
        'Рамка 1-я': 724 + 265,
        'Рамка 2-я': 360,
        'Рамка 3-я': 148,
        'Рамка 4-я': 140,
...
# Пришло на объект, но в проекте такого нет: производственная необходимость,
# замены, довозы. В ведомости идут отдельным разделом с проектом «—».
EXTRA = {
    'b1': [
        {'n': 'Рамка 5-я горизонтальная белая', 'u': 'шт', 'recv': 30},
        {'n': 'Выключатель трёхклавишный скрытой проводки, белый 10А, IP20', 'u': 'шт', 'recv': 25},
    ],
}
...
def main():
    ...
    for bid, items in RECEIVED.items():
        ...
        for x in EXTRA.get(bid, []):
            out[x['n'] + '|' + x['u']] = x['recv']
        # приводим к единому виду: список поставок
        norm = {}
        for k, v in out.items():
            if isinstance(v, list):
                norm[k] = [{'q': d['q'], 'd': d.get('d', ''), 'doc': d.get('doc', '')} for d in v]
            else:
                norm[k] = [{'q': v, 'd': '', 'doc': 'учтено до перехода на накладные'}]
        if norm:
            recv[bid] = norm
        if unmatched:
    ...
    pkg = {
        't': 'uch-object', 'v': 1,
        'dlv': recv,
        'extra': {k: [{'n': x['n'], 'u': x['u']} for x in v] for k, v in EXTRA.items()},
        'object': OBJECT_NAME,
        'buildings': buildings,
        'preset': preset,
    }
